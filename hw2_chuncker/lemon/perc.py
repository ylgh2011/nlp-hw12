...
def perc_possiblity_dict(tagset, train_data):
    unigram_dict = {}
    bigram_dict = {}
    trigram_dict = {}
    tag_amount = {}
...
def perc_test(feat_vec, labeled_list, feat_list, tagset, default_tag, pos_dict):
    ...
    for i in range(2, N-2):
        ...
        last_best = get_maxvalue(viterbi[i])[0]
...
def perc_testall(feat_vec, data, tagset, pos_dict):
    ...
    for (labeled_list, feat_list) in data:
        ...
        # data format [(['Rockwell NNP','International NNP',...],['U00:_B-2','U01:_B-1','U02:Rockwell',...] ), (...), ...]
        output = perc_test(feat_vec, labeled_list, feat_list, tagset, default_tag, pos_dict)
...
if __name__ == '__main__':
    ...
    optparser.add_option("-n", "--inputfile", dest="inputfile", default=os.path.join("data", "input.txt.gz"), help="input data, i.e. the x in \phi(x,y)")
    optparser.add_option("-e", "--infeatfile", dest="ifeatfile", default=os.path.join("data", "input.feats.gz"), help="precomputed features for the input data, i.e. the values of \phi(x,_) without y")
    ...
    test_data = read_labeled_data(opts.inputfile, opts.ifeatfile)
    train_data = read_labeled_data(opts.trainfile, opts.featfile)
    pos_dict = perc_possiblity_dict(tagset, train_data)
    print >>sys.stderr, "done."
    feat_vec = perc_read_from_file(opts.modelfile)
    perc_testall(feat_vec, test_data, tagset, pos_dict)
