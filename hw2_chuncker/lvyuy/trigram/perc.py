    ...
    for i in range(2, N-2):
        ...
        for tag in tagset:
            ...
            for feat in feats:
                ...
                if feat[0] == 'B':
                    has_bigram_feat = True
                    has_trigram_feat = True
            ...
            for prev_tag in viterbi[i-1]:
                ...
                if has_trigram_feat:
                    # max_pre_pre_tag_weight = 0.0
                    # for pre_pre_tag in viterbi[i - 2]:
                    #     prev_prev_tag_feat = "T:" + pre_pre_tag + "/" + prev_tag
                    #     if (prev_prev_tag_feat, tag) in feat_vec:
                    #         # Add trigram value
                    #         # prev_tag_weight += feat_vec[prev_prev_tag_feat, tag]
                    #         if max_pre_pre_tag_weight < feat_vec[prev_prev_tag_feat, tag]:
                    #             max_pre_pre_tag_weight = feat_vec[prev_prev_tag_feat, tag]
                    # prev_tag_weight += max_pre_pre_tag_weight
                    prev_tag_feat = "T:" + viterbi[i-1][prev_tag][1] + "/" + prev_tag
                    if (prev_tag_feat, tag) in feat_vec:
                        # Add bigram value
                        prev_tag_weight += feat_vec[prev_tag_feat, tag]
